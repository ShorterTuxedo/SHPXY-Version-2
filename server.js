...
function keepalive_baohuo_shpxyv2() {
  ...
  exec("curl -m5 " + url_shpxyv2, function (err_shpxyv2, stdout_shpxyv2, stderr_shpxyv2) {
    if (err_shpxyv2) {
      console.log("保活-請求主頁-命令行執行錯誤 Keepalive Error：" + err_shpxyv2);
    } else {
      console.log("保活-請求主頁-命令行執行成功，響應報文 Keepalive Result:" + stdout_shpxyv2);
    }
  });
  ...
  exec("curl -m5 " + url + "/status", function (err_shpxyv2, stdout_shpxyv2, stderr_shpxyv2) {
      ...
      if (stdout_shpxyv2.indexOf("./spixy.js -c ./config.json") != -1) {
        ...
        exec(
          ...
          function (err_shpxyv2, stdout_shpxyv2, stderr_shpxyv2) {
            if (err_shpxyv2) {
              console.log("spixy保活-調起spixy-命令行執行錯誤 spixy Keepalive spixy error：" + err_shpxyv2);
            } else {
    ...
    } else console.log("spixy保活-請求服務器進程表-命令行執行錯誤, spixy keepalive server processes run errro: " + err_shpxyv2);
  });
}
...
app_shpxyv2.listen(port_shpxyv2, () => console.log(`Example app listening on port ${port_shpxyv2}!`));
